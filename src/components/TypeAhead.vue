<template>
	<div :id="wrapperId" class="simple-typeahead">
		...
		<div v-if="isListVisible" class="simple-typeahead-list">
			...
			>
				<span class="simple-typeahead-list-item-text" :data-text="itemProjection(`${item.firstName} ${item.lastName}`)" v-if="$slots['list-item-text']"
					><slot name="list-item-text" :item="item" :itemProjection="itemProjection" :boldMatchText="boldMatchText"></slot
				></span>
				<span class="simple-typeahead-list-item-text" :data-text="itemProjection(`${item.firstName} ${item.lastName}`)" v-html="boldMatchText(itemProjection(`${item.firstName} ${item.lastName}`))" v-else></span>
			</div>
	...
	export default /*#__PURE__*/ defineComponent({
		...
		methods: {
			...
			selectItem(item) {
				...
				this.input = this.itemProjection(name);
		...
		computed: {
			...
			filteredItems() {
				...
				return this.items.filter((item) => this.itemProjection(`${item.firstName} ${item.lastName}`).match(regexp));
			},
			isListVisible() {
